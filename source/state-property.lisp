...
(define-model-class state-property (property)
  ...
  (:maker #f)
  (:documentation "A state property links a finite state machine with an entity as a property. The state of the finite state machine can be queries via the state property as a property."))
...
(def method shared-initialize :around ((state-property direct-state-property) slot-names
                                      &rest args &key state-machine &allow-other-keys)
  (bind ((state-machine
          (aif (typep state-machine 'finite-state-machine)
               state-machine
               (find-finite-state-machine state-machine))))
    (assert state-machine)
    (apply #'call-next-method state-property slot-names
           :state-machine state-machine
           :type (state-type-of state-machine)
           args)))
...
(def method initialize-instance :after ((state-property state-property) &rest args)
  ...
  (declare (ignore args))
  (bind ((start-state (start-state-of (state-machine-of state-property))))
...
(def method hu.dwim.perec::compute-persistent-effective-slot-definition-option ((class entity)
                                                                                 (direct-slot direct-state-property)
                                                                                slot-option-name
...
(def method (setf slot-value-using-class) (value
                                          (class entity)
                                          (object persistent-object)
                                          (state-property effective-state-property))
  (if (typep value 'state)
      (call-next-method)
      (let ((state (find-state (state-machine-of state-property) value)))
        (assert (typep state 'state))
